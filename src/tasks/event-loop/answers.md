...
setTimeout(() => console.log("2: setTimeout 0"), 0); таймаут попадает в web api и раз у нас задержка 0 то его коллбек почти сразу переходит в очередь МАКРОзадач которые выполнятся в фазе timers

setImmediate(() => console.log("3: setImmediate")); плохо знаю что делает setImmediate потому что еще не использовал такое, но прочитав в интерненте понял что setImmediate кладет свой коллбек в очередь МАКРОзадач и выполняет их в фазе check

(на данный момент у нас уже две МАКРОзадачи)
...
}); чтение файла переносит ВЕСЬ вот этот большой коллбек в очередь МАКРОзадач который выполнится в фазе Poll I/O фаза
...
Когда все микрозадачи закончились то начинается фаза timers и выполнение МАКРОзадачи
первая в очереди стоит () => console.log("2: setTimeout 0")
по этому в консоли мы видим 
"2: setTimeout 0"

После этого node js проверяет есть ли у нас МИКРОзадачи, и раз их нету то он проверяет есть ли у нас МАКРОзадачи setTimeout, раз их нету, переходит в фазу poll, он должен выполнить функцию из fn, но она еще не прочитал все файлы, по этому он переходит на фазу check и приступает к выполнению новой МАКРОзадачи
() => console.log("3: setImmediate") и в консоль выводится 
"3: setImmediate"

Далее опять проверка есть ли МИКРОзадачи, и раз их нету и нету больше setImmediate и таймеров то он переходит на фазу poll, где он уже прочитал файлы и выполняет МАКРОзадачу от fn
() => {
  console.log("7: readFile callback"); 
...
Далее после фазы poll начинается фаза check в которой выполняются все setImmediate коллбеки
() => console.log("9: inner setImmediate") и выводит в консоль 
"9: inner setImmediate"

Далее проверяет очередь МИКРОзадач, и раз она пустая, то после фазы check он вернулся в timers, берет новую задачу из МАКРОзадач
...
В интернете на это, сказали что это случайность потому что код из setTimeout выполнился не сразу и то что хоть и стоит что 0ms задержка у него, он все равно может задерживаться на 1-4 ms. И он не успел выполнится и начал выполнятся setImmediate, по этому они и поменялись местами. 

P.P.S. Понадобилось время, но я разобрался. setTimeout находится в фазе timers, а setImmediate в фазе check. Если setTimeout не успевает пройти, то начинается новая фаза event loop. Немного подправил объяснение